...
export default function AdminExchangePage() {
      ...
      setSelected((prev) =>
        prev
          ? {
              ...prev,
              status,
              admin_memo: adminMemo.trim() || prev.admin_memo,
            }
          : null
      );
    }
  ...
  const filtered = requests.filter((r) => {
    ...
    return (
      r.request_number?.toLowerCase().includes(keyword) ||
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} className="p-2 rounded-xl hover:bg-gray-100" type="button">
      ...
      <div className="relative">
        ...
        <input
          className="w-full h-11 pl-10 pr-4 rounded-xl border-2 border-gray-200 text-sm focus:border-orange-400 outline-none"
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`status-chip ${STATUS_COLORS[r.status]}`}>{r.status}</span>
                  </td>
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-2xl p-4 text-sm space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-mono font-bold">{selected.request_number}</span>
              </div>

              <div className="flex justify-between">
                <span className="text-gray-500">고객</span>
                <span className="font-bold">
                  {selected.customers?.name} {selected.customers?.phone}
                </span>
              </div>

              <div className="flex justify-between">
                <span className="text-gray-500">유형</span>
                <span className="font-bold">{selected.request_type}</span>
              </div>

              <div className="flex justify-between">
                <span className="text-gray-500">상태</span>
                <span className={`status-chip ${STATUS_COLORS[selected.status]}`}>
                  {selected.status}
                </span>
              </div>
            </div>
